...
public class Service {
	...
	private Node getSCPDNode() {
		...
		Log.e("Service:getSCPDNode", "scpdURLStr = " + scpdURLStr);
		try {
			URL scpdUrl = new URL(rootDev.getAbsoluteURL(scpdURLStr));
			Log.e("Service:getSCPDNode", "scpdUrl = " + scpdUrl.toString());
			...
			Log.e("Service:getSCPDNode",
					"exception in getSCPDNode1: " + e.getMessage());
			Debug.warning(e);
		}

		String newScpdURLStr = rootDev.getDescriptionFilePath()
				+ HTTP.toRelativeURL(scpdURLStr);
		Log.e("Service:getSCPDNode", "newScpdURLStr = " + newScpdURLStr);
		try {
			scpdNode = getSCPDNode(new File(newScpdURLStr));
		} catch (Exception e) {
			Log.e("Service:getSCPDNode",
					"exception in getSCPDNode2: " + e.getMessage());
			Debug.warning(e);
		}
	...
	public ServiceStateTable getServiceStateTable() {
		...
		Node stateTableNode = getSCPDNode()
				.getNode(ServiceStateTable.ELEM_NAME);
