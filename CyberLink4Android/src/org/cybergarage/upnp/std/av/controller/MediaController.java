...
public class MediaController extends ControlPoint {
	...
	public Node browse(Device dev, String objectID, String browseFlag,
			String filter, int startIndex, int requestedCount,
			String sortCaiteria) {
		...
		/*
		 * ContentDirectory:1 Service Template Version 1.01 2.7.4.2. Argument
		 * Descriptions RequestedCount ui4 Requested number of entries under the
		 * object specified by ObjectID. RequestedCount =0 indicates request all
		 * entries. Added to set the RequestedCount parameter using the
		 * NumberReturned result when the specified parameter is zero and the
		 * NumberReturned parameter is less than the TotalMatches parameter for
		 * XMBC.
		 */
		if (requestedCount == 0) {
	...
	public Node browseMetaData(Device dev, String objectId, String filter,
			int startIndex, int requestedCount, String sortCaiteria) {
		return browse(dev, objectId, BrowseAction.BROWSE_METADATA, filter,
				startIndex, requestedCount, sortCaiteria);
	}
	...
	public Node browseDirectChildren(Device dev, String objectID,
			String filter, int startIndex, int requestedCount,
			String sortCaiteria) {
		return browse(dev, objectID, BrowseAction.BROWSE_DIRECT_CHILDREN,
	...
	public ContainerNode browse(Device dev, String objectId,
			boolean hasBrowseChildNodes, boolean hasRootNodeMetadata) {
	...
	private int browse(ContainerNode parentNode, Device dev, String objectID,
			boolean hasBrowseChildNodes) {
		if (objectID == null)
			return 0;
	...
	public void printMediaServers() {
		...
		for (int n = 0; n < devCnt; n++) {
			...
			if (dev.isDeviceType(MediaServer.DEVICE_TYPE) == false)
				continue;
			System.out.println("[" + n + "] " + dev.getFriendlyName() + ", "
	...
	public boolean setAVTransportURI(Device dev, ItemNode itemNode) {
		...
		return setAVTransportURI(dev, resURL);
	}

	public boolean setAVTransportURI(Device dev, String resURL) {
		if (dev == null)
			return false;
